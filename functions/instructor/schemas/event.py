...
class GetInstructorsParams(TypedDict):
    p: NotRequired[int]
    rp: NotRequired[int]
...
# class CreateInstructorEventBody:
#     name: str
#     identity_number: str
#     gender: str
#     phone_number: str
...
#         if body.keys() != names:
#             raise HttpError(
#                 400, "INVALID_BODY", "Instructor fields are missing or unknown"
#             )
#         if any(not isinstance(body[name], str) for name in names):
#             raise HttpError(400, "INVALID_BODY", "Instructor fields must be strings")
...
# def instructor_id(event: dict) -> UUID:
