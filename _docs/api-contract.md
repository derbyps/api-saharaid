...
```ts
Paginated<Pick<Participant,
  "id" | "serial_number" | "name" | "phone_number" | "email" | "created_at"
>>
```

The list never contains documents or signed URLs.

### GET `/participant?id={uuid}`
...
```ts
{ instructor: Instructor; document: DocumentMetadata | null }
```
...
```ts
{ course: { id: string; sanity_id: string; asset_status: "ready" } }
```
...
{
  schedule: Schedule;
  participants: Array<Pick<Participant,
...
Paginated<{
  ...
  schedule: Pick<Schedule,
    "id" | "course_id" | "course_title" | "class" | "batch" | "start_date" | "end_date"
  >;
}>
```
